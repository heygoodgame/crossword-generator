...
def _make_mini_7_catalog(
    ...
    *,
    ...
    center_optional = {tuple(sorted(pattern)) for pattern in center_optional_patterns}
...
_GRID_PATTERNS: dict[
    ...
    (PuzzleType.MINI, 5): _filter_accepted_symmetry_patterns(
        5,
        [
            # --- 9x frequency ---
            ([], 9),
    ...
    *,
    ...
    Args:
        ...
        seed: Optional seed to randomly select a black cell pattern
              ...
              pattern as default.
        exclude_open: Drop patterns with no black cells (all-white grids)
              from the catalog before selection. Used for Starter puzzles,
              where an open 5x5 forces ten 5-letter answers.
    ...
    if pattern_data and exclude_open:
        pattern_data = [
            (black_cells, weight) for black_cells, weight in pattern_data if black_cells
        ]
    if pattern_data:
        if seed is not None:
